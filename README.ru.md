...
- **APT-пакеты:** `git`, `less`, `mariadb-client`, `msmtp`,
  `openssh-client`, `unzip`. `msmtp` симлинкован как `/usr/sbin/sendmail`,
  чтобы `mail()` PHP работал из коробки.

Глобальный Drush в образ не устанавливается. Поддерживаемый способ
запускать Drush — проектная копия из Composer-проекта: `vendor/bin/drush`.
...
| `<версия>-php<X.Y>` | `:0.1.0-php8.4` | конкретный релиз на ветке PHP |
